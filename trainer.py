...
from frames import Frames, RunFrame
...
class Trainer(object):
  def __init__(self,
    ...
    self.local_network = Model(self.action_size,
                                     thread_index,
                                     use_pixel_change,
                                     use_value_replay,
                                     use_reward_prediction,
  ...
  def _process_pc(self, sess):
    ...
    if not pc_experience_frames[1].terminal:
      pc_R = self.local_network.run_pc_q_max(sess,
                                             ...
                                             pc_experience_frames[0].last_action_reward(self.action_size))
    ...
    for frame in pc_experience_frames[1:]:
      ...
      a = np.zeros([self.action_size])
      a[frame.action] = 1.0
      last_action_reward = frame.last_action_reward(self.action_size)
  ...
  def _process_vr(self, sess):
    ...
    if not vr_experience_frames[1].terminal:
      vr_R = self.local_network.run_vr_value(sess,
                                             ...
                                             vr_experience_frames[0].last_action_reward(self.action_size))
